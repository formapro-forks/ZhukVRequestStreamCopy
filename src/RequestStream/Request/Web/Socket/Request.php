...
class Request extends WebAbstract implements RequestInterface {
  ...
  protected function initSocket()
  {
    ...
    return $this->socket->setTransport('tcp')
          ->setPort(is_null($this->uri['port']) ? 80 : $this->uri['port']);
  }
  ...
  protected function initSocketSSL()
  {
    ...
    return $this->socket->setTransport('ssl')
          ->setPort(is_null($this->uri['port']) ? 443 : $this->uri['port']);
  }
  ...
  protected function createRequest()
  {
    ...
    if ($this->proxy) {
      ...
      $this->socket->setTarget($this->uri['host']);
    }
  ...
  protected function writeHeaderToSocket()
  {
    ...
    $write = $this->getMethod() . ' ' . $this->uri['path'] . ($this->uri['query'] ? '?' . $this->uri['query'] : '') . ' HTTP/' . $this->getHttpVersion() .  "\r\n";
    $write .= 'Host: ' . $this->uri['host'] . "\r\n";
